...
{
    /// <summary>
    /// The ordinary case on the route the documenter is really used through: an assembly
    ...
    /// Default one.
    ///
    /// Nothing the documenter reads is supposed to differ, which is exactly why it is
    /// pinned: an unmanaged step is created with the plugin type on EventHandler and the
    /// platform is what fills in PluginTypeId, and the documenter's step query joins on
